...
#define SPATIALSOFTMAX_THREADS 128

// Parallelization across each feature point.
__global__ void cunn_SpatialSoftMax_updateOutput_kernel
 ...
 float constant)
{
  ...
  if (idx < data_size) {
    ...
    // sum
    for(int i = idx; i < next_idx; i += spatial_size) {
      sum += __expf(input[i]-max);
      output[i] = __expf(input[i] - max);
    }
...
__global__ void cunn_SpatialSoftMax_updateGradInput_kernel(float *gradInput, float *output, float *gradOutput, int feature_size, int spatial_size, int data_size)
{
  int idx = (threadIdx.x + blockDim.x*blockIdx.x);
  idx = (idx/spatial_size)*feature_size + idx % spatial_size;
  ...
  if (idx < data_size) {
    ...
    for(int i = idx; i < next_idx; i += spatial_size){
      gradSum += gradOutput[i]*output[i];
    }
    // Compute the new gradient
    for(int i = idx; i < next_idx; i += spatial_size){
      gradInput[i] = (gradOutput[i]- gradSum)*output[i];
    }
  }
}
...
// THCudaTensor_resizeAs(output, input);
  ...
  int nblocks = nthreads/SPATIALSOFTMAX_THREADS;
  if (nthreads % SPATIALSOFTMAX_THREADS != 0){
    nblocks = nblocks + 1;
  }

  dim3 blocks(nblocks,1,1);
  dim3 threads(SPATIALSOFTMAX_THREADS,1,1);

  cunn_SpatialSoftMax_updateOutput_kernel<<<blocks,threads>>>
...
// THCudaTensor_resizeAs(gradInput, output);
  ...
  cunn_SpatialSoftMax_updateGradInput_kernel<<<blocks,threads>>>(THCudaTensor_data(gradOutput),
...
{
luaL_openlib(L, "nn", cunn_SpatialSoftMax__, 0);
}
